...
class ScreenCaptureThread(QThread):
    ...
    def _process_cv2(self, frame_bgra):
    ...
    def _process_pil(self, sct_img):
        img = Image.frombytes("RGB", sct_img.size, sct_img.bgra, "raw", "BGRX")
        if self.blur_radius > 0:
            img = img.filter(ImageFilter.GaussianBlur(radius=self.blur_radius))
        if self.gray_mode:
            gray = img.convert("L").convert("RGB")
            img  = Image.blend(img, gray, self.gray_level / 255.0)
        data = img.tobytes("raw", "RGB")
        return QImage(data, img.size[0], img.size[1], QImage.Format_RGB888)

    def run(self):
        if not MSS_AVAILABLE:
            return

        use_cv2 = CV2_AVAILABLE and NUMPY_AVAILABLE
                ...
                while self.running:
                    ...
                    try:
                        ...
                        if use_cv2:
                            ...
                            processed  = self._process_cv2(frame_bgra)
                            h, w       = processed.shape[:2]
                            qimg = QImage(processed.data, w, h, w * 3,
                                          QImage.Format_RGB888)
                        else:
                            qimg = self._process_pil(sct_img)
    ...
    def _setup(self):
        try:
            ...
            def on_press(key):
                ...
                elif vk and vk in vk_actions:
                    self._emit(vk_actions[vk])
